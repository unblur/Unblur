...
import { setAuthErrors, setAuthUser, setAuthMessages } from '../reducers/actions';
...
const verifyEmail = (token, id, onComplete) => async dispatch => {
    try {
        const res = await axios.get(`http://localhost:8000/api/users/verifyemail?token=${token}&id=${id}`)
        dispatch({ type: setAuthMessages, payload: [res.data.message] })
    } catch (e) {
